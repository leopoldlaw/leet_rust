 ...
 */
pub fn mergeTwoLists(l1: Option<Box<ListNode>>, l2: Option<Box<ListNode>>) -> Option<Box<ListNode>> {
    match  (l1, l2) {
        ...
        (Some(l), Some(r)) => {
            if l.val <= r.val {
                return Some(Box::new(ListNode{ next: mergeTwoLists(l.next, Some(r)), val: l.val }));
            } else {
                return Some(Box::new(ListNode{ next: mergeTwoLists(Some(l), r.next), val: r.val }));
            }
        },
    }
}
...
mod tests {
    ...
    #[test]
    fn valid_anagram1() {
        assert_eq!(valid_anagram::is_anagram(String::from("rat"),String::from("cat")),false);
    }
    #[test]
    fn valid_anagram2() {
        assert_eq!(valid_anagram::is_anagram(String::from("anagram"),String::from("nagaram")),true);
    }
    #[test]
    fn binary_search1() {
    ...
    fn lowest_common_ancestor1(){
        ...
        assert_eq!(
            lowest_common_ancestor::lowest_common_ancestor(root.clone(), node5.clone(), node1.clone()),
            root
        );
    ...
    fn balanced_binary_tree1(){
        ...
        assert_eq!(
            balanced_binary_tree::is_balanced(root.clone()),
            true
        );
